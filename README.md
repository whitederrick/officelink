...
### 리뷰/글
`/review/new` `/review/[id]` `/post/[id]` `/write` `/channel/[id]`

### 모드별
`/landlord` `/manager` `/landlord-dashboard` `/manager-dashboard` `/fees` `/rent`
...
✓ src/lib/i18n.test.ts      (11 tests)
✓ src/lib/pwa.test.ts       (7 tests)
...
| **v0.6** | 단기임대 + 그룹채팅 + 이벤트 + PWA 풀세트 + 테스트 | **60+** |
